  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onClick={() => toast.success('Navigate to Leads to convert organizations into active Companies.')} className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-xl text-sm font-medium transition-all shadow-lg shadow-indigo-500/25 flex items-center gap-2">
      ...
      <div className="glass-panel p-3 rounded-2xl flex flex-wrap gap-3 items-center">
        ...
        <button onClick={() => toast.success('Filter criteria applied to companies list.')} className="px-4 py-2 bg-slate-800/50 hover:bg-indigo-500/20 hover:text-indigo-300 text-slate-300 border border-indigo-500/10 rounded-xl text-sm font-medium transition-colors flex items-center gap-2">
          ...
          {companies.map((company, i) => (
            ...
            >
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => toast.success('Company options expanded.')} className="p-2 text-slate-400 hover:text-white bg-slate-900/80 rounded-xl backdrop-blur-sm transition-colors">
