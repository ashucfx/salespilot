...
public class LeadService {
    ...
    public LeadDto createLead(CreateLeadRequest request, User currentUser) {
        ...
        if (request.getAssignedToId() != null) {
            assignedTo = employeeRepository.findById(request.getAssignedToId()).orElseThrow(() -> new ResourceNotFoundException("Employee", request.getAssignedToId()));
        } else {
        ...
        lead.setContactDesignation(request.getContactDesignation());
        ...
        lead.setContactLinkedin(request.getContactLinkedin());
        ...
        lead.setCurrency(request.getCurrency() != null ? request.getCurrency() : "INR");
        lead.setInterestedServices(request.getInterestedServices());
        ...
        if (assignedTo != null && !assignedTo.getUser().getId().equals(currentUser.getId())) {
            ...
            emailService.sendLeadAssignedEmail(assignedTo.getWorkEmail(), assignedTo.getFirstName(), lead.getContactName(), lead.getCompanyName());
        }
    ...
    public LeadDto updateLead(UUID id, UpdateLeadRequest request, User currentUser) {
        ...
        if (request.getAssignedToId() != null && (lead.getAssignedTo() == null || !lead.getAssignedTo().getId().equals(request.getAssignedToId()))) {
            Employee newAssignee = employeeRepository.findById(request.getAssignedToId()).orElseThrow(() -> new ResourceNotFoundException("Employee", request.getAssignedToId()));
            lead.setAssignedTo(newAssignee);
            if (!newAssignee.getUser().getId().equals(currentUser.getId())) {
                emailService.sendLeadAssignedEmail(newAssignee.getWorkEmail(), newAssignee.getFirstName(), lead.getContactName(), lead.getCompanyName());
            }
        }
    ...
    private Lead getLeadOrThrow(UUID id) {
        return leadRepository.findById(id).filter(l -> l.getDeletedAt() == null).orElseThrow(() -> new ResourceNotFoundException("Lead", id));
    }
    ...
    public LeadService(final LeadRepository leadRepository, final LeadSourceRepository leadSourceRepository, final EmployeeRepository employeeRepository, final NotificationService notificationService, final EmailService emailService) {
