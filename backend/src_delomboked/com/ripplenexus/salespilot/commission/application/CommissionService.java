...
public class CommissionService {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(CommissionService.class);
    ...
    public Commission calculateAndCreate(Deal deal, Employee employee) {
        ...
        commission.setDealValue(deal.getDealValue());
        commission.setCommissionAmount(commissionAmount);
        commission.setCurrency(deal.getCurrency());
        commission.setStatus(Commission.CommissionStatus.PENDING);
        commission = commissionRepository.save(commission);
        log.info("Commission created: {} for employee: {} | Amount: {}", commission.getId(), employee.getEmployeeNumber(), commissionAmount);
        return commission;
    }
    ...
    public void updateEmployeeCommissionRule(UUID employeeId, BigDecimal newPercentage) {
        ...
        CommissionRule rule = commissionRuleRepository.findByName(ruleName).orElseGet(() -> {
            ...
            newRule.setPercentage(newPercentage);
            newRule.setActive(true);
            return commissionRuleRepository.save(newRule);
        });
        commissionPlanRepository.findByEmployeeIdAndIsActiveTrue(employeeId).forEach(plan -> {
            plan.setActive(false);
            plan.setEffectiveTo(LocalDate.now());
            commissionPlanRepository.save(plan);
        });
        EmployeeCommissionPlan newPlan = new EmployeeCommissionPlan();
        newPlan.setEmployee(employee);
        newPlan.setRule(rule);
    ...
    public void approve(UUID commissionId, UUID approverEmployeeId, String notes) {
        ...
        Employee approver = employeeRepository.findById(approverEmployeeId).orElseThrow(() -> new ResourceNotFoundException("Employee", approverEmployeeId));
        ...
        notificationService.notifyCommissionApproved(commission.getEmployee().getUser(), formattedAmount);
    ...
    public void reject(UUID commissionId, UUID approverEmployeeId, String reason) {
        ...
        commissionRepository.save(commission);
        notificationService.notifyCommissionRejected(commission.getEmployee().getUser(), reason);
    ...
    public void markPaid(UUID commissionId, UUID paidByEmployeeId, String paymentRef) {
        ...
        Employee paidBy = employeeRepository.findById(paidByEmployeeId).orElseThrow(() -> new ResourceNotFoundException("Employee", paidByEmployeeId));
        ...
        commissionRepository.save(commission);
        String formattedAmount = formatAmount(commission.getCommissionAmount(), commission.getCurrency());
        notificationService.notifyCommissionPaid(commission.getEmployee().getUser(), formattedAmount);
        emailService.sendCommissionPaidEmail(commission.getEmployee().getWorkEmail(), commission.getEmployee().getFirstName(), formattedAmount);
    ...
    public PageResponse<Commission> getByEmployee(UUID employeeId, String status, Pageable pageable) {
        if (status != null && !status.isBlank()) {
            return PageResponse.of(commissionRepository.findByEmployeeIdAndStatus(employeeId, Commission.CommissionStatus.valueOf(status.toUpperCase()), pageable));
        }
    ...
    public PageResponse<Commission> getAll(String status, Pageable pageable) {
        if (status != null && !status.isBlank()) {
            return PageResponse.of(commissionRepository.findByStatus(Commission.CommissionStatus.valueOf(status.toUpperCase()), pageable));
        }
    ...
    public PageResponse<EmployeePayoutSummaryDto> getPayoutSummary(Pageable pageable) {
        return PageResponse.of(employeeRepository.findAllActive(pageable).map(emp -> {
            ...
            EmployeePayoutSummaryDto summary = new EmployeePayoutSummaryDto();
            summary.setEmployeeId(emp.getId());
            summary.setEmployeeName(emp.getFullName());
            summary.setEmployeeNumber(emp.getEmployeeNumber());
            summary.setTotalPendingCommission(pending != null ? pending : BigDecimal.ZERO);
            summary.setLeadsGenerated(leadsGen);
            summary.setDealsClosed(dealsClosed);
            summary.setNextPayoutDate(endOfMonth.toString());
            return summary;
        }));
    }
    ...
    BigDecimal calculateAmount(CommissionRule rule, BigDecimal dealValue) {
        return switch (rule.getType()) {
            case PERCENTAGE -> dealValue.multiply(rule.getPercentage()).divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
            case FIXED -> rule.getFixedAmount();
            case HYBRID -> {
                BigDecimal pct = dealValue.multiply(rule.getPercentage()).divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
    ...
    private BigDecimal calculateTieredAmount(List<CommissionTier> tiers, BigDecimal dealValue) {
        for (CommissionTier tier : tiers) {
            ...
            boolean belowMax = tier.getMaxAmount() == null || dealValue.compareTo(tier.getMaxAmount()) <= 0;
            if (aboveMin && belowMax) {
                if (tier.getPercentage() != null) {
                    BigDecimal pct = dealValue.multiply(tier.getPercentage()).divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
    ...
    private CommissionRule getActiveRuleForEmployee(Employee employee) {
        return commissionPlanRepository.findActiveRuleByEmployee(employee.getId(), LocalDate.now()).orElseGet(() -> commissionRuleRepository.findFirstByIsActiveTrueOrderByCreatedAtAsc().orElseThrow(() -> new BusinessException("No active commission rule found.")));
    }

    private Commission getOrThrow(UUID id) {
        return commissionRepository.findById(id).filter(c -> c.getDeletedAt() == null).orElseThrow(() -> new ResourceNotFoundException("Commission", id));
    }
    ...
    public CommissionService(final CommissionRepository commissionRepository, final CommissionRuleRepository commissionRuleRepository, final CommissionPlanRepository commissionPlanRepository, final EmployeeRepository employeeRepository, final LeadRepository leadRepository, final NotificationService notificationService, final EmailService emailService) {
