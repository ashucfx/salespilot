...
public class PipelineService {
    ...
    public List<PipelineEntry> getEntries(com.ripplenexus.salespilot.auth.domain.User currentUser) {
        ...
        return allEntries.stream().filter(e -> e.getLead().getAssignedTo() != null && e.getLead().getAssignedTo().getUser().getId().equals(currentUser.getId())).toList();
    }

    public void updateLeadStage(UUID leadId, UUID newStageId, int position, com.ripplenexus.salespilot.auth.domain.User currentUser) {
        ...
        PipelineStage stage = stageRepository.findById(newStageId).orElseThrow(() -> new ResourceNotFoundException("Stage", newStageId));
        PipelineEntry entry = entryRepository.findByLeadId(leadId).orElse(new PipelineEntry());
        entry.setLead(lead);
        entry.setStage(stage);
        entry.setEnteredAt(Instant.now());
        entry.setPosition(0);
        entry.setDaysInStage(0);
    ...
    public PipelineService(final PipelineStageRepository stageRepository, final PipelineEntryRepository entryRepository, final LeadRepository leadRepository, final EmployeeRepository employeeRepository) {
