...
public class ClientForgeController {
    ...
    public ResponseEntity<ApiResponse<Void>> handleInvoicePaidWebhook(@RequestHeader(value = "X-Client-Forge-Signature", required = false) String signature, @RequestBody InvoicePaidWebhookRequest request) {
        ...
        if (signature == null || !signature.equals(EXPECTED_SECRET)) {
            return ResponseEntity.status(org.springframework.http.HttpStatus.UNAUTHORIZED).body(ApiResponse.error("Invalid webhook signature"));
        }
    ...
    public ClientForgeController(final ClientForgeService clientForgeService) {
        this.clientForgeService = clientForgeService;
    }
}
