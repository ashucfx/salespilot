...
public class IncentiveService {
    ...
    public List<IncentiveDto> getAllActiveIncentives() {
        return incentiveRepository.findByStatusAndDeletedAtIsNull(Incentive.Status.ACTIVE).stream().map(IncentiveDto::from).toList();
    }

    public List<EmployeeIncentiveDto> getEmployeeIncentives(UUID userId) {
        Employee employee = getEmployeeByUserId(userId);
        evaluateIncentivesForEmployee(employee);
        return employeeIncentiveRepository.findByEmployeeIdAndDeletedAtIsNull(employee.getId()).stream().map(EmployeeIncentiveDto::from).toList();
    }

    public EmployeeIncentiveDto claimIncentive(UUID userId, UUID incentiveId) {
        Employee employee = getEmployeeByUserId(userId);
        evaluateIncentivesForEmployee(employee);
        EmployeeIncentive ei = employeeIncentiveRepository.findByEmployeeIdAndIncentiveIdAndDeletedAtIsNull(employee.getId(), incentiveId).orElseThrow(() -> new ResourceNotFoundException("Incentive progress not found for employee"));
    ...
    public List<LeaderboardDto> getLeaderboard() {
        ...
        for (Employee emp : employees) {
            ...
            BigDecimal claimedRewards = empIncentives.stream().filter(ei -> ei.getStatus() == EmployeeIncentive.ClaimStatus.CLAIMED).map(ei -> ei.getIncentive().getRewardAmount()).reduce(BigDecimal.ZERO, BigDecimal::add);
            ...
            dto.setAvatarUrl(emp.getProfilePicture());
            dto.setDesignation(emp.getDesignation() != null ? emp.getDesignation() : "Sales Executive");
            dto.setTotalRevenue(totalRev);
            dto.setDealsClosed(dealsClosed);
            dto.setTotalIncentivesEarned(claimedRewards);
            dto.setSalesTier(tier);
            dto.setBadgeIcon(getTierIcon(tier));
            leaderboard.add(dto);
        }
    ...
    public void evaluateIncentivesForEmployee(Employee employee) {
        ...
        for (Incentive inc : activeIncentives) {
            EmployeeIncentive ei = employeeIncentiveRepository.findByEmployeeIdAndIncentiveIdAndDeletedAtIsNull(employee.getId(), inc.getId()).orElseGet(() -> {
                EmployeeIncentive eiNew = new EmployeeIncentive();
                eiNew.setEmployee(employee);
                eiNew.setIncentive(inc);
                eiNew.setCurrentProgress(BigDecimal.ZERO);
                eiNew.setStatus(EmployeeIncentive.ClaimStatus.IN_PROGRESS);
                return eiNew;
            });
            ...
            if (ei.getStatus() == EmployeeIncentive.ClaimStatus.IN_PROGRESS && progress.compareTo(inc.getTargetValue()) >= 0) {
    ...
    private Employee getEmployeeByUserId(UUID userId) {
        return employeeRepository.findByUserId(userId).orElseThrow(() -> new ResourceNotFoundException("Employee profile not found for user " + userId));
    }

    @java.lang.SuppressWarnings("all")
    
    public IncentiveService(final IncentiveRepository incentiveRepository, final EmployeeIncentiveRepository employeeIncentiveRepository, final EmployeeRepository employeeRepository, final DealRepository dealRepository, final LeadRepository leadRepository, final EmailService emailService) {
        this.incentiveRepository = incentiveRepository;
        this.employeeIncentiveRepository = employeeIncentiveRepository;
