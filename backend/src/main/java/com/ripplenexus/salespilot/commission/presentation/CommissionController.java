...
public class CommissionController {
    ...
    public ResponseEntity<ApiResponse<PageResponse<Commission>>> getEmployeeCommissions(@PathVariable UUID employeeId, @org.springframework.security.core.annotation.AuthenticationPrincipal com.ripplenexus.salespilot.auth.domain.User currentUser, Pageable pageable) {
        boolean isAdminOrManager = currentUser.getRoles().stream().anyMatch(r -> r.getName().equals("ADMIN") || r.getName().equals("SALES_MANAGER"));
        if (!isAdminOrManager) {
            com.ripplenexus.salespilot.employee.domain.Employee employee = employeeRepository.findByUserId(currentUser.getId()).orElseThrow(() -> new com.ripplenexus.salespilot.core.exception.ResourceNotFoundException("Employee profile not found"));
    ...
    public ResponseEntity<ApiResponse<Void>> approveCommission(@PathVariable UUID id, @org.springframework.security.core.annotation.AuthenticationPrincipal com.ripplenexus.salespilot.auth.domain.User currentUser) {
    ...
    @PostMapping("/{id}/pay")
    @PreAuthorize("hasRole(\'ADMIN\')")
    public ResponseEntity<ApiResponse<Void>> payCommission(@PathVariable UUID id, @org.springframework.security.core.annotation.AuthenticationPrincipal com.ripplenexus.salespilot.auth.domain.User currentUser) {
    ...
    public CommissionController(final CommissionService commissionService, final com.ripplenexus.salespilot.employee.infrastructure.EmployeeRepository employeeRepository) {
